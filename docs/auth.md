...
**The discovery surface (trip feed and search) is no longer in this list (Phase 6, SOCIAL-01).** It used to permit anonymous browsing of `PUBLIC` trips; it now requires a Bearer token like everything else, because SOCIAL-01's "For You" feed requires a signed-in account to browse at all, and carving out only the feed endpoint while leaving the others public would still let a stranger enumerate every `PUBLIC` trip. See `docs/api-contracts.md`'s Discovery section for the corrected per-endpoint auth requirement.
