 ...
 * place at the same instant used to turn one of them into a 500 — {@code createPlace()}'s
 * duplicate-key recovery ran in the same transaction as the failed insert, and a real Postgres
 * constraint violation aborts that whole transaction, so the "recovery" read failed too
 * ({@code current transaction is aborted, commands ignored until end of transaction block}).
 * {@link com.tripflow.backend.service.PlaceResolutionServiceTest} only proves the recovery
 * logic against a mocked repository, which never exercises a real aborted transaction and so
 * could not have caught this. This test forces a genuine race against Testcontainers Postgres.
 *
 ...
 * {@link AuthConcurrencyIT} / {@link RouteOptimizationConcurrencyIT}.
 */
...
class PlaceResolutionConcurrencyIT {
	...
	void createTrip_concurrentRequestsReferencingSamePlace_neitherReturns500() throws Exception {
		...
		String externalPlaceId = "race-place-" + System.nanoTime();
